...
def character_chat(request, character_id):
    ...
    messages = Chat.objects.filter(character=character, user=request.user).order_by('timestamp')
...
def send_message(request, character_id):    
    if request.method == 'POST':
        ...
        chat = Chat.objects.create(character=character, user=request.user, user_input=message_text, character_response=chatbot_response)
        ...
        #     # Get the messages for the specified character
        #     messages = Chat.objects.filter(character=character, user=request.user)

        #     # return template fragment with all the messages between user and character
